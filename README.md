...
> **Why three?** You asked for "send normally → the strong model sees only the enhanced
...
## Verified against current behavior (2026‑06‑02, Claude Code 2.1.154)
...
- **The proxy works with subscription auth.** Verified live: under `claude.ai` (Max) auth,
  Claude Code forwards its `Authorization` header to a local `ANTHROPIC_BASE_URL` proxy,
  which relays it upstream unchanged — and the strong model received only the rewritten
  prompt (`72 → 369` chars, confirmed via metadata log).
